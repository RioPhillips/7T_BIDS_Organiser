"""fixepi - Fix EPI JSON metadata (PhaseEncodingDirection, TotalReadoutTime)."""
...
def run_fixepi(studydir: Path, subject: str, session: Optional[str] = None,
    ...
    fmap_dir, sourcedata_dir = sess.paths["fmap"], sess.paths["sourcedata"]
    if not fmap_dir.exists():
        logger.warning("fmap directory not found"); return
    if not sourcedata_dir.exists():
        logger.warning("sourcedata directory not found"); return
...
def _update_direction_jsons(fmap_dir, sourcedata_dir, sess, direction, ped, force, logger):
    ...
    if not series_dirs:
        logger.warning(f"No DICOM series found for {direction}"); return
    ...
    for json_file in json_files:
        ...
        try:
            ...
            wfs = ds[0x2001, 0x1022].value
            imf = ds[0x0018, 0x0084].value
            epf = ds[0x2001, 0x1013].value
        except Exception as e:
            logger.error(f"Error reading DICOM: {e}"); continue
